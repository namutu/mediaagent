...
public class MediaAgentUtil {
	...
	private Logger log = LoggerFactory.getLogger(MediaAgentUtil.class);
...
//		}
	...
	public File createSRTFile(File file) {
		...
		String SRTFileName = FilenameUtils.removeExtension(file.getAbsolutePath()) + ".srt";
		
		String command = "subs -c srt '" + file.getAbsolutePath() + "' -o '" + SRTFileName + "'";
		
		log.info("SRT ��ȯ ��ɾ� [" + command + "]");
		
		String succ = callShell(command);
		
		if(!succ.equals(MAConsts.FAIL)) {
